...
Date: 2025-12-01 12:09:16
LastEditors: 汪培良 rick_wang@yunquna.com
LastEditTime: 2025-12-01 14:16:20
FilePath: /RAG_service/chain/dashscope_pipeline.py
...
class DashScopeMLModel:
    ...
    def predict(self, text:str) -> str:
        ...
        try:
            ...
            print(f"Error calling DashScope model: {e}")
            return "unknown"
...
class TongyiLLMRouter:
    ...
    def predict(self, text: str, intents: List[str]) -> str:
         ...
         try:
            ...
            print(f"Error in TongyiLLMRouter predict: {e}")
            return "unknown"
...
class DashScopeIntentPipeline:
    ...
    def _init_role_engine(self):
        """初始化规则引擎"""
        if self.config.get("enable_rule_engine", True):
            self.role_engine = RoleEngine(self.config['role_engine'])
        else:
    ...
    def _init_ml_model(self):
        ...
        if self.config.get("enable_ml_model", True):
            ...
            self.ml_model = DashScopeMLModel(
                model_name = ml_config.get("model_name", "qwen-plus"),
                api_key = ml_config.get("api_key", ""),
                base_url = ml_config.get("base_url", "https://dashscope.aliyuncs.com/api/v1")
            )
            self.ml_model = IntentClassifier(self.config['ml_model'])
        else:
            self.ml_model = None
    ...
    def _create_pipeline(self):
        ...
        if self.rule_engine:
            runnables["rule_engine"] = lambda x : self.rule_engine.predict(x['input'])

        if self.ml_model:
            runnables["ml_model"] = lambda x : self.ml_model.predict(x['input'])

        if self.llm_router:
            runnables["llm_router"] = lambda x : self.llm_router.predict(x['input'], self.intents)
    ...
    def predict(self, text:str) -> Dict[str, Any]:
        ...
        try:
            # 并行执行各种方法
            result = self.parallel_router.invoke({"input": text})
            # 投票决定最终结果
            final_intent = self.voting_logic.vote(result)
