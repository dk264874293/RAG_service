...
LastEditTime: 2025-12-18 13:27:44
...
async def main() -> None:
    ...
    agent1 = AssistantAgent(
        'Assistant_1', model_client=model_client,
        description="For calculation",
        system_message="Calculate the sum of two numbers",
    )

    agent2 = AssistantAgent(
        'Assistant_2', model_client=model_client,
        tools=[check_calculation],
        description="For checking the calculation",
    ...
    def selector_func(messages:Sequence[BaseAgentEvent | BaseChatMessage]) -> str | None:
        if len(messages) == 1 or messages[-1].to_text() == "Incorrect":
            return "Assistant_1"
        if messages[-1].source == "Assistant_1":
            return "Assistant_2"
        return None
