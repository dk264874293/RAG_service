...
class CustomPromptTemplate(StringPromptTemplate):
    ...
    """
   # 模版参数定义
    template_type: str =  Field(default='person_analysis', description="模版类型")
    include_skills_analysis: bool = Field(default=True, description="是否包含技能分析")
    include_career_advice: bool = Field(default=True, description="是否包含职业建议")
    output_language: str = Field(default='zh', description="输出语言")
    ...
    def _build_base_info_section(self,person_info: PersonInfo) -> str:
        ...
        if person_info.location:
            info_lines.append(f"- 所在地区: {person_info.location}")

        if person_info.skills:
            info_lines.append(f"- 技能: {', '.join(person_info.skills)}")
