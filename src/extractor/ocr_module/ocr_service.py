...
class OCRService:
    ...
    def __init__(
        self, config_path: Optional[str] = None, engine: Optional[str] = None, **kwargs
    ):
        """
        初始化OCR服务
    ...
    def recognize(
        ...
        **kwargs,
        ...
        # 准备输入数据（保持原始类型，避免不必要的转换）
        image_data = self._prepare_input(image_input)

        # 预处理（仅对本地引擎的 ndarray 输入有效）
        if preprocess or (preprocess is None and self.config.preprocess_enabled):
            if self.preprocessor and isinstance(image_data, np.ndarray):
                image_data = self.preprocessor.process(image_data)

        # OCR识别
        try:
            raw_results = self.ocr_engine.recognize(image_data, **kwargs)
        except Exception as e:
        ...
        **kwargs,
        ...
        if parallel and len(image_inputs) > 1:
            return self._recognize_parallel(
                image_inputs, preprocess, postprocess, **kwargs
            )
        else:
            results = []
    ...
    def _recognize_parallel(self, image_inputs, preprocess, postprocess, **kwargs):
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            future_to_idx = {
                executor.submit(
                    self.recognize, img_input, preprocess, postprocess, **kwargs
                ): idx
    ...
    def _prepare_input(
        ...
        """
        准备输入数据，根据 OCR 引擎类型决定如何处理

        关键修复：避免总是转换为 ndarray，保持原始类型
        - 远程 API 模式：需要 bytes/str/dict
        - 本地引擎模式：支持 str/np.ndarray/Image.Image

        Args:
            image_input: 原始输入数据

        Returns:
            处理后的输入数据（保持原始类型或必要的转换）
        """
        # 保持原始类型，让底层 OCR 引擎决定如何处理
    ...
    def _load_image(self, image_input):
        """
        加载图像为numpy数组（已废弃，保留以兼容旧代码）

        注意：此方法不再被 recognize() 使用，因为它总是将输入转换为 ndarray，
        这会破坏远程 API 模式的类型要求（需要 bytes/str/dict）

        请使用 _prepare_input() 代替
        """
        self.logger.warning("_load_image() 已废弃，请使用 _prepare_input()")
